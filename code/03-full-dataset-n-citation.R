...
# from 00-data-prep.R
verified_occ <- st_read("data/verified_occurrences_2025-04-21.gpkg")
unverified_occ <- st_read("data/unverified_occurrences_2025-04-21.gpkg")
...
# Add columns from species master Google Sheet---------------------------------------
...
# Add 'origin', 'Tip_SpeciesName', and 'OTU' to data
occ_w_master_df <- occ_w_dup_info |>
  left_join(species_master |>
    select(species, Origin, Tip_SpeciesName, OTU), by = c("species")) |>
  as_tibble()

# Add road distance ---------------------------------------
road_dist_df <- read_csv("data/occ_nearest_trail_2025-04-22-2.csv")
final_occ_df <- occ_w_master_df |>
  mutate(gbifid = as.numeric(gbifid)) |>
  left_join(road_dist_df |> select(gbifid, distance_to_nearest_trail = distance),
    by = "gbifid"
  )
...
final_occ_df |>
  filter(
    verifiedTaxon == "verified",
    keepWhenDeduplicating == "keep",
    distance_to_nearest_trail |> is.na()
  )
...
final_occ_df |>
  select(-geom) |>
  write_csv("data/full_gbif_for_analysis.csv")

final_occ_df |> colnames()
...
derived_dataset_prep(
  ...
  source_url = "https://zenodo.org/records/15265921"
)
